...
# ============================================================
# INPUT TABLE (ONE ROW, START BLANK) + RESET
# ============================================================
DEFAULT_EMPTY = pd.DataFrame([{
    ...
    "Door_Style": "Classic",
    "Fixed_Door_Width_mm": 762,  # workaround: column stays visible but only used when Housebuilder == Avant
}])
...
st.sidebar.write(f"Base side liner thickness: **{BASE_SIDE_LINER_THICKNESS} mm** (MTM unless builder forces 50)")
...
st.caption("Note: **Fixed door width** is only used when **Housebuilder = Avant** (workaround).")
...
edited_df = st.data_editor(
    ...
    column_config={
        ...
        "Fixed_Door_Width_mm": st.column_config.SelectboxColumn(
            "Fixed door width (mm) (Avant only)",
...
# ============================================================
# CALCULATION FUNCTION (single row)
# ============================================================
def calculate_for_row(row: pd.Series) -> pd.Series:
    ...
    if door_system == "Fixed 2223mm doors":
        ...
        # Workaround: only read the Fixed_Door_Width_mm when Avant is selected
        if hb == "Avant":
            door_width = float(row.get("Fixed_Door_Width_mm", 762))
        else:
            door_width = 762.0  # safe default when not Avant
...
# Only show warnings/download if we have inputs
if results_df.loc[0, "Issue"] != "—":
    ...
    st.info("Enter Width and Height in Section 1 to calculate results.")
...
with col2:
    ...
    st.write(f"**Fixed door width used:** {row.get('Fixed_Door_Width_Used', '')}")
